    ...
    public class PatInfoSynchronous : BaseJobAction, IDisposable
    {
        ...
        private void AddNewRecord(DataTable targetTable, DataRow sourceRow, bool isPatient)
        {
            ...
            foreach (DataColumn column in sourceRow.Table.Columns)
            {
                ...
                if (isPatient)
                {
                    if (column.ColumnName == colPatientState.ToUpper())
                    {
                ...
                else if (column.ColumnName.Equals(colBedState.ToUpper()))
                {
        ...
        private string MergePatientInfo(DataTable changedData, bool initialized)
        {
            ...
            try
            {
                foreach (DataRow hisRow in changedData.Rows)
                {
                    ...
                    matchRows = m_EmrPatientTable.Select(String.Format(" PATNOOFHIS = '{0}' ", hisRow[colHisFirstPageNo].ToString().ToUpper().Trim()), " PATNOOFHIS asc ");
        ...
        private int SaveEmrTableData(DataTable sourceTable, string tableName)
        {
            ...
            if (sourceTable != null)
            {
                ...
                if (m_NewPatients.Count > 0)
                {
                    ...
                    //YidanSoft.JobManager.JobLogHelper.WriteSqlLog(Parent, "select syxh, hissyxh from " + EMRPatTable + " where hissyxh in (" + condition.ToString() + ")");
        ...
        private DataSet GetPatientAndBedTable(bool isInit)
        {
            try
            {
                ...
                if (isInit)
                {
                    ...
                    if (depts.Length > 0)
                        sqlInpatient = " select * from YD_INPATIENT where (admitdept in ({0}) or outhosdept in ({0})  )and STATUS IN(1500,1501) ";
                    else
                        sqlInpatient = " select * from YD_INPATIENT where  STATUS IN(1500,1501,1503) ";
                    ...
                    //读取所有床位
                    DataTable bed = m_HisHelper.ExecuteDataTable(" select * from YD_BEDS ");
                    bed.TableName = "bed";
                    ds.Tables.Add(bed.Copy());
                    //commandText = "usp_Emr_GetInpatients";
                ...
                {
                    //读取当日所有患者(在院患者、两日内出院患者)
                    //add by yxy 目前情况无法导入转科到指定科室的病人  如原先心内科的病人转科到眼科，但是入院科室为心内科无法读取。
                    if (depts.Length > 0)
                        sqlInpatient = @" select * from YD_INPATIENT where (admitdept in ({2}) or outhosdept in ({2}) ) and STATUS IN(1500,1501) 
                                             union 
                                          select *from  YD_INPATIENT where (admitdept in ({2}) or outhosdept in ({2}) ) 
                                                    and STATUS IN(1502,1503) and (outwarddate between '{0}' and '{1}')";
                    else
                        sqlInpatient = @" select * from YD_INPATIENT where STATUS IN(1500,1501) 
                                             union 
                                             select *from  YD_INPATIENT where 
                                                       (STATUS IN(1502,1503) and (outwarddate between '{0}' and '{1}'))";
                    DataTable inpatient = this.m_HisHelper.ExecuteDataTable(string.Format(sqlInpatient, DateTime.Now.AddDays(-1.0).ToString("yyyy-MM-dd"),
